...
class FG_eval {
 public:
  ...
  void operator()(ADvector &fg, const ADvector &vars) {
    ...
    // `fg` a vector of the cost constraints, and constraints
    ...
    for (int i = 0; i < N-1; i++) {
      ...
      fg[2 + i + psi_start] = psi1 - (psi0 + v0 / Lf * delta * dt);
      fg[2 + i + v_start] = v1 - (v0 + a * dt);
      fg[2 + i + cte_start] = cte1 - (fx0 - y0 + v0 * CppAD::sin(epsi0) * dt);
      fg[2 + i + epsi_start] = epsi1 - (psi0 - psides0 + v0 / Lf * delta * dt);
    }
  }
};
